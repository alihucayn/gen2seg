...
from util.conv_layer import RGBToGrayscale
import transformers
...
def evaluate_validation(model, vae, val_dataloader, accelerator, weight_dtype, args, convLayer=None):
    """
    ...
    Args:
        ...
        convLayer: Optional convolution layer for channel conversion (only used with BCE loss)
    
    Returns:
        Dictionary of validation metrics
    """
    model.eval()
    convLayer.eval() if convLayer is not None else None
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(val_dataloader, desc="Validating", total=total, disable=not accelerator.is_local_main_process):
            ...
            # Apply conv layer to convert from 3 channels to 1 channel (if provided)
            if convLayer is not None:                
                current_estimate = convLayer(current_estimate)
                # For BCE loss - first apply sigmoid to convert logits to probabilities
                current_estimate = torch.sigmoid(current_estimate) * 255.0  # Scale to [0, 255]
            else:
            ...
            for i in range(current_estimate.shape[0]):
                ...
                # Convert to grayscale if needed (only if convLayer wasn't used)
                if pred_np.ndim == 3 and pred_np.shape[0] in [3, 1]:
                    pred_np = pred_np.mean(axis=0)
                if target_np.ndim == 3 and target_np.shape[0] in [3, 1]:
    ...
    model.train()
    convLayer.train() if convLayer is not None else None
...
def main():
    ...
    # Only create convLayer if using BCE loss (which requires single channel output)
    convLayer = None
    if args.loss_type == "bce":
        convLayer = RGBToGrayscale(trainable=True)
    ...
    # Freeze VAE and CLIP (text encoder), train only UNet and convLayer (if using BCE)
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                for i, model in enumerate(models):
                    if hasattr(model, 'save_pretrained'):  # This is the UNet
                        model.save_pretrained(os.path.join(output_dir, "unet"))
                    else:  # This is the convLayer
                        torch.save(model.state_dict(), os.path.join(output_dir, "conv_layer.pth"))
                    weights.pop()
        ...
        def load_model_hook(models, input_dir):
            for i in range(len(models)):
                ...
                if hasattr(model, 'save_pretrained'):  # This is the UNet
                    ...
                    conv_layer_path = os.path.join(input_dir, "conv_layer.pth")
                    if os.path.exists(conv_layer_path):
                        model.load_state_dict(torch.load(conv_layer_path, map_location="cpu"))
    ...
    # Combine parameters from UNet and convLayer (if it exists)
    trainable_params = list(unet.parameters())
    if convLayer is not None:
        trainable_params += list(convLayer.parameters())
    ...
    if args.dataset_type == "rtm":
        ...
        train_dataset = RTMDataset(
            ...
            grayscale_labels=True if args.loss_type == "bce" else False
        )
    ...
    if convLayer is not None:
        unet, convLayer, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            unet, convLayer, optimizer, train_dataloader, lr_scheduler
        )
    else:
    ...
    if accelerator.mixed_precision == "fp16":
        ...
        if convLayer is not None:
            convLayer.to(dtype=weight_dtype)
    ...
    if args.loss_type == "bce":
        # BCEWithLogitsLoss combines a sigmoid with binary cross-entropy,
        # so we pass in raw logits (not sigmoid-activated values)
        loss_fn = torch.nn.BCEWithLogitsLoss(reduction="mean")
    elif args.loss_type == "dice":
        loss_fn = diffusers.losses.DiceLoss()
    elif args.loss_type == "bseg":
        loss_fn = BinarySegmentationLoss()
    elif args.loss_type == "bce_seg":
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # Save the RGB current estimate before applying convLayer
                rgb_current_estimate = current_estimate.clone()
                
                # Apply conv layer to convert from 3 channels to 1 channel (only for BCE loss)
                if convLayer is not None:
                    current_estimate = convLayer(current_estimate)  # This outputs logits for BCE
                ...
                if args.report_to == "wandb" and accelerator.is_main_process and global_step % args.wandb_image_interval == 0:
                    with torch.no_grad():
                        ...
                        # For visualization: if using BCE loss (convLayer), apply sigmoid and scale to [0, 255]
                        if convLayer is not None:
                            img_to_log = (torch.sigmoid(current_estimate[0].detach().cpu()) * 255).clamp(0, 255).to(torch.uint8)
                            # Also log the RGB estimate before convLayer
                        ...
                        in_img_to_log = ((batch["image"][0].detach().cpu() + 1.0) / 2.0 * 255.0).clamp(0, 255).to(torch.uint8)
                ...
                # Compute loss
                if args.loss_type == "bce":
                    # For BCE loss, ground_truth should be in [0, 1] range
                    # and current_estimate should be logits (unbounded)
                ...
                if accelerator.sync_gradients:
                    ...
                    if convLayer is not None:
                        trainable_params = list(unet.parameters()) + list(convLayer.parameters())
            ...
            if accelerator.sync_gradients:
                ...
                if args.loss_type == "bseg":
                    ...
                    accelerator.log({"black_loss": loss_fn.blackLoss}, step=global_step)
                    accelerator.log({"separation_loss": loss_fn.separationLoss}, step=global_step)
                    accelerator.log({"bce_loss": loss_fn.bceLoss}, step=global_step)
                ...
                if val_dataloader is not None and global_step % args.validation_steps == 0:
                    ...
                    val_metrics, sample_preds, sample_targets = evaluate_validation(
                        ...
                        convLayer=unwrap_model(convLayer) if convLayer is not None else None
                    )
            ...
            if 'rgb_current_estimate' in locals():
                del rgb_current_estimate
    ...
    if accelerator.is_main_process:
        ...
        if convLayer is not None:
            convLayer = unwrap_model(convLayer)
        ...
        # Save the convLayer separately (only if it exists)
        if convLayer is not None:
            conv_layer_path = os.path.join(args.output_dir, "conv_layer.pth")
            torch.save(convLayer.state_dict(), conv_layer_path)
            logger.info(f"Saved convLayer to {conv_layer_path}")
